...
import { addUserToGroup, attachGroupToUser, createNewGroup } from "./api";
...
const HomePage = ({ setShowNavBar }) => {
  ...
  function handleJoinGroup() {
    ...
    kidsProfiles.forEach((kp) => {
      addUserToGroup(groupCode.toUpperCase(), kp.userName, kp.displayName);
      attachGroupToUser(groupCode.toUpperCase(), kp.userName);
    });
  }
  ...
  return (
    <Wrapper>
      ...
      {usersGroupNames && (
        <>
          {usersGroupNames.length === 0 ? (
            <GroupsHeader>
              You are currently not a part of any groups. Either join an
              existing group or create a new group using the buttons below
            </GroupsHeader>
          ) : (
            <>
              <GroupsHeader>
                You are currently a part of the following group(s){" "}
              </GroupsHeader>
      ...
      <ButtonWrapper>
        <StyledButton
          ...
          selected={showForm === "join"}
        >
          JOIN EXISTING GROUP
        </StyledButton>
        <StyledButton
          onClick={() => setShowForm("create")}
          selected={showForm === "create"}
        >
          CREATE NEW GROUP
        </StyledButton>
      </ButtonWrapper>
            ...
            <>
              <div>
                <StyledInput
                  ...
                  upperCase={true}
                />
                <StyledLabel hasValue={groupCode !== ""}>
                  Group code
                </StyledLabel>
              </div>
              <ApiButton
                disabled={
                  !allGroups.some((g) => g.code === groupCode.toUpperCase())
                }
              ...
              {!allGroups.some((g) => g.code === groupCode.toUpperCase()) && (
                <WarningText>Group code does not exist</WarningText>
              )}
            </>
          ) : (
            <>
              ...
              <div>
                ...
                />
                <StyledLabel hasValue={groupName !== ""}>
                  Group name
                </StyledLabel>
              </div>
              <ApiButton
              ...
              {allGroups.some((g) => g.code === groupCode.toUpperCase()) && (
                <WarningText>Group code already exists</WarningText>
              )}
            </>
          )}
        </FormWrapper>
