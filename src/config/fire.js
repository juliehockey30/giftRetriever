...
class Firebase {
  ...
  createKidUser(userName, parentUserName) {
    return this.db.ref(`users/${userName}`).set({
      parentProfile: parentUserName,
    });
  }

  addKidProfileToParentProfile(userName, kidsUserName, kidsDisplayName) {
    return this.db.ref(`users/${userName}/kidsProfiles`).push({
      kidsUserName: kidsUserName,
      kidsDisplayName: kidsDisplayName,
    });
  }

  getKidsProfilesForParent(userName) {
    return this.db.ref(`users/${userName}/kidsProfiles`);
  }
}
